...
def distorted_inputs(data_dirs, batch_size):
  ...
  for dir in data_dirs:
    filenames = [join(dir, f) for f in listdir(dir) if (isfile(join(dir, f)) and FLAGS.test_batch not in f )] # and 'data_batch_50.bin' not in f
  ...
  size = tf.cast(tf.concat((scale*height, scale*width), axis=0),tf.int32)
  resized_image = tf.image.resize_images(images=float_image,size=size) 
